// DESIGN: IPCollabCollectionFactory is the single deploy point for all
// IPCollabCollection contracts. Anyone can deploy a new collab collection —
// the caller becomes its owner: the only address that can create contribution
// types, manage verifiers, and (with its verifiers) review submissions. The
// factory is fully immutable and ownerless: the collection class hash is
// fixed at deploy time. Mirrors IPTicketCollectionFactory (IP-Tickets).
...
pub mod IPCollabCollectionFactory {
    ...
    use crate::interface::{IIPCollabCollectionFactory, IIP_COLAB_COLLECTION_FACTORY_ID};
    ...
    struct Storage {
        ...
        /// Class hash used to deploy new IPCollabCollection instances.
    ...
    /// Emitted each time a new IPCollabCollection is deployed via
    /// `deploy_collection`.
    #[derive(Drop, starknet::Event)]
    ...
    fn constructor(ref self: ContractState, collection_class_hash: ClassHash) {
        ...
        self.src5.register_interface(IIP_COLAB_COLLECTION_FACTORY_ID);
        self.collection_class_hash.write(collection_class_hash);
        // deploy_nonce defaults to 0 — no explicit write needed
    }

    #[abi(embed_v0)]
    impl IPCollabCollectionFactoryImpl of IIPCollabCollectionFactory<ContractState> {
