...
pub trait IMaliciousERC20Config<TContractState> {
    fn configure_attack(
        ref self: TContractState, target: ContractAddress, negotiation_id: u256, mode: u8,
    );
}
...
mod MaliciousERC20 {
    use ip_negotiation_escrow::interface::{
        IIPNegotiationEscrowDispatcher, IIPNegotiationEscrowDispatcherTrait,
    };
    use ip_negotiation_escrow::malicious_erc20::IMaliciousERC20Config;
    use ip_negotiation_escrow::mock_erc20::IERC20Mint;
    ...
    const ATTACK_SELLER_CLAIM: u8 = 1;
    const ATTACK_BUYER_REFUND: u8 = 2;
    ...
    impl ConfigImpl of IMaliciousERC20Config<ContractState> {
        fn configure_attack(
            ref self: ContractState, target: ContractAddress, negotiation_id: u256, mode: u8,
        ) {
            self.target.write(target);
            self.negotiation_id.write(negotiation_id);
    ...
    impl InternalImpl of InternalTrait {
        fn maybe_attack_transfer(ref self: ContractState) {
            ...
            let escrow = IIPNegotiationEscrowDispatcher { contract_address: target };
            if mode == ATTACK_SELLER_CLAIM {
                escrow.claim_seller_funds(self.negotiation_id.read());
            }
            if mode == ATTACK_BUYER_REFUND {
                escrow.claim_buyer_refund(self.negotiation_id.read());
            }
        }
    }
}
