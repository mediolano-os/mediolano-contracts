...
use ip_club::interface::{
    IIPClubCollectionDispatcher, IIPClubCollectionDispatcherTrait,
    IIPClubCollectionFactoryDispatcher, IIPClubCollectionFactoryDispatcherTrait,
    IIP_CLUB_COLLECTION_FACTORY_ID,
};
...
fn deploy_factory() -> ContractAddress {
    ...
    let factory_class = declare("IPClubCollectionFactory").unwrap().contract_class();
...
fn test_deploy_collection_returns_address() {
    ...
    let col = factory.deploy_collection("Band Club", "BAND", "ipfs://QmClubMeta/");
...
fn test_deployed_collection_owner_is_caller() {
    ...
    let caller = deploy_mock_account();
    start_cheat_caller_address(factory_addr, caller);
    let col_addr = factory.deploy_collection("My Club", "CLUB", "ipfs://QmMeta/");
    stop_cheat_caller_address(factory_addr);
    // The caller can create a membership without panic — they own the collection.
    let club = IIPClubCollectionDispatcher { contract_address: col_addr };
    start_cheat_caller_address(col_addr, caller);
    let id = club.create_membership(10_u256, Option::None, Option::None, 0_u16, "ipfs://QmTest");
    stop_cheat_caller_address(col_addr);
    assert(id == 1_u256, 'first tier id = 1');
}
...
fn test_deployed_collection_carries_base_uri() {
    ...
    let caller = deploy_mock_account();
    start_cheat_caller_address(factory_addr, caller);
    let col_addr = factory.deploy_collection("My Club", "CLUB", "ipfs://QmCollMeta/");
    stop_cheat_caller_address(factory_addr);
    let club = IIPClubCollectionDispatcher { contract_address: col_addr };
    assert(club.name() == "My Club", 'name propagated');
    assert(club.symbol() == "CLUB", 'symbol propagated');
    assert(club.base_uri() == "ipfs://QmCollMeta/", 'base_uri propagated');
}
...
fn test_two_deployments_have_different_addresses() {
    ...
    let caller = deploy_mock_account();
    start_cheat_caller_address(factory_addr, caller);
    let col1 = factory.deploy_collection("Club A", "CLBA", "ipfs://QmA/");
    let col2 = factory.deploy_collection("Club B", "CLBB", "ipfs://QmB/");
    stop_cheat_caller_address(factory_addr);
...
fn test_different_callers_get_different_addresses() {
    ...
    let col1 = factory.deploy_collection("Club", "CLB", "ipfs://QmT/");
    stop_cheat_caller_address(factory_addr);
    start_cheat_caller_address(factory_addr, caller2);
    let col2 = factory.deploy_collection("Club", "CLB", "ipfs://QmT/");
    stop_cheat_caller_address(factory_addr);
...
fn test_factory_src5_registered() {
    ...
    assert(src5.supports_interface(IIP_CLUB_COLLECTION_FACTORY_ID), 'factory SRC5 registered');
}
...
fn test_deploy_empty_name_panics() {
    ...
    let caller = deploy_mock_account();
    start_cheat_caller_address(factory_addr, caller);
    factory.deploy_collection("", "CLB", "ipfs://QmT/");
}
...
fn test_deploy_empty_symbol_panics() {
    ...
    let caller = deploy_mock_account();
    start_cheat_caller_address(factory_addr, caller);
    factory.deploy_collection("My Club", "", "ipfs://QmT/");
}
